...
const getPendingRequests=async(req,res,next)=>{
    try{
        ...
        for(let i=0;i<request.length;i++){
            ...
            r={loanData:{...request[i]},borrowerData:{...borrower_details}}
            data.push(r)
        }
        res.status(200).json({
            success:true,
...
const updateStatus=async(req,res,next)=>{
    try{
        ...
        const updatedRequest=await Loan.findByIdAndUpdate(requestId,{
            status:"accepted",
            createdAt:Date.now()
        },{
            new:true
        ...
        const updated_lender_details=await User.findByIdAndUpdate(req.user.id,{
...
const lenderHistory=async(req,res,next)=>{
    try{
        ...
        for(let i=0;i<data.length;i++){
            ...
            let months_pending=data[i].createdAt?.getMonth()+parseInt(data[i].months)-new Date().getMonth()
            let interest_amount=((parseFloat(data[i].interestrate)/100)*parseFloat(data[i].amount))*parseInt(data[i].months)
            ...
            finalData.push({
                name,phoneNumber,email,address,document,months_pending,interest_amount,amount_pending,interestrate,principalamount,total_amount
            })
        }
        res.status(200).json({
            success:true,
...
const repay=async(req,res,next)=>{
    try{
        ...
        let months_remaing=loan.createdAt?.getMonth()+parseInt(loan.months)-new Date().getMonth()
        let remaining_interest_amount=loan.interest_remaining
        let current_interest=(parseFloat(loan.interestrate/100)*parseFloat(loan.amount))*months_remaing
        let amount_remaing;
        ...
        if(remaining_interest_amount<current_interest){
            ...
            let s=amount-(parseFloat(loan.interestrate/100)*parseFloat(loan.amount))
            if(s>0){
        ...
        let updatedRequest=await Loan.findByIdAndUpdate(requestId,{
